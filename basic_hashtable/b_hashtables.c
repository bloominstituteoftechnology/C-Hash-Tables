...
{
  int hash_at_key = hash(key, ht->capacity);
  if (ht->storage[hash_at_key] != NULL){
    destroy_pair(ht->storage[hash_at_key]);
    ht->storage[hash_at_key] = NULL;
  }
  return ht;
}
...
{
  int hash_at_key = hash(key, ht-> capacity);
  if(ht->storage[hash_at_key]){
    return ht->storage[hash_at_key]->value;
  }
  return NULL;
}
